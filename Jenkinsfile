            ...
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'webserver_server_login', usernameVariable: 'USERNAME', passwordVariable: 'USERPASS')]) {
                    script {
                        ...
                        sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@$production_server_ip \"docker run --restart always --name train-schedule -p 8080:8080 -d reihudec/train-schedule:${env.BUILD_NUMBER}\""
                    }
                }
            }
        }
